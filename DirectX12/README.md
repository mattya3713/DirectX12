...
- `@brief`には「このファイルが何をするか」だけを書く。他プロジェクトからの移植であることや経緯はここに書かない(必要ならコミットメッセージやDESIGN.mdに書く)。
- デザインパターンを採用している場合は`@pattern`行を追加し、パターン名だけを簡潔に書く。

```cpp
/**********************************************************************************
* @author    : mattya3713.
* @date      : 2026/08/10.
* @brief     : 型ごとにサービス(非所有ポインタ)を登録・取得するクラス.
