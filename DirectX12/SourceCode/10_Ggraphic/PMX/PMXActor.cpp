...
void PMXActor::Draw() {
	...
	// 4. マテリアルごとの描画ループ
	unsigned int idxOffset = 0; // インデックスバッファの開始オフセット

	for (int i = 0; i < m_ModelData.Materials.size(); ++i)
	{
		unsigned int numFaceVertices = m_ModelData.Materials[i].NumFaceCount; // このマテリアルの描画に使用するインデックス数
		...
		idxOffset += numFaceVertices; // 次のマテリアルの開始インデックスへオフセットを進める
	}
}
