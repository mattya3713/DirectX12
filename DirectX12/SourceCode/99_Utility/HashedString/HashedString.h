...
public:
	...
	// ハッシュ値の取得.
	constexpr HashID GetHash() const noexcept { return m_Hash; }
...
private:
	...
	static constexpr HashID Hash(std::string_view Str) noexcept
	{
		HashID hash = 2166136261u;
		for (char c : Str) {
			hash ^= static_cast<HashID>(static_cast<unsigned char>(c));
			hash *= 16777619u;
		}
		return hash;
	}
