...
*            : (位置・回転・スケール)、敵スポーン(定義ID+Transform)、
*            : Player/Bossスポーン地点を編集し、Data\Json\Level配下のJSONへ
*            : 保存/読込するImGuiツール. 保存・読込時にOnLevelChangedコールバック
*            : を飛ばすため、MainSceneはそれを受けてレベルを再構築できる.
*            : 当たり判定・地形高低差は対象外(見た目の配置のみ).
*            : 敵スポーンはデータ契約のみ(実体生成はEnemyFactory側タスク).
**********************************************************************************/
...
private:
	...
	std::vector<std::string>    m_CatalogIds;        // 敵定義カタログのID一覧(コンボ候補).

	std::vector<LevelObjectDesc>      m_Objects;          // 編集中の配置オブジェクト.
	std::vector<LevelEnemySpawnDesc>  m_EnemySpawns;      // 編集中の敵スポーン.
