...
std::wstring DirectWriteCustomFont::GetFontName(int num)
{
	...
	// リストのサイズを超えていた場合
	if (num >= static_cast<int>(fontNameList.size()))
	{
		return fontNameList[0];
	}

	return fontNameList[num];
}
...
// options：テキストの整形
//====================================
HRESULT DirectWriteCustomFont::DrawString(std::string str, D3DXVECTOR2 pos, bool shadow, D2D1_DRAW_TEXT_OPTIONS options)
{
	HRESULT result = S_OK;
...
HRESULT DirectWriteCustomFont::GetFontFamilyName(IDWriteFontCollection* customFontCollection, const WCHAR* locale)
{
	...
	for (UINT32 i = 0; i < familyCount; i++)
	{
		...
		// フォントファミリー名を取得して name に追加.
		std::wstring name;
		name.resize(length + 1); // 文字列の長さに応じてメモリを確保.
